...
UPDATE $ltm_translations SET is_deleted = ? WHERE is_deleted = $whereClause AND "group" = ? AND "key" LIKE ?
SQL
        ), [$value, $group, $key]);
    }
    ...
    {
        $ltm_translations = $this->getTranslationsTableName();
        $this->translation->getConnection()->update($this->adjustTranslationTable("UPDATE $ltm_translations SET \"group\" = ?, \"key\" LIKE ?, status = 1 WHERE id = ?"), [$group, $key, $id]);
    }
...
SELECT source FROM $ltm_translations WHERE "group" = ? AND "key" LIKE ?
SQL
        ), [$group, $key]);
    }
 ...
 . implode(",", $values));
    ...
    public function updatePublishTranslations($newStatus, $group = null, $locale = null)
    {
        ...
        if ($group) {
            if ($locale) {
                $this->translation->getConnection()->affectingStatement($this->adjustTranslationTable(<<<SQL
UPDATE $ltm_translations SET saved_value = "value", status = ?, is_auto_added = FALSE WHERE (COALESCE(saved_value,'') <> COALESCE("value",'') || (status <> ? and status <> ?)) AND "group" = ? AND locale = ?
SQL
                ), [$newStatus, $newStatus, Translation::STATUS_SAVED, $group, $locale]);
            } else {
                $this->translation->getConnection()->affectingStatement($this->adjustTranslationTable(<<<SQL
UPDATE $ltm_translations SET saved_value = "value", status = ?, is_auto_added = FALSE WHERE (COALESCE(saved_value,'') <> COALESCE("value",'') || (status <> ? and status <> ?)) AND "group" = ?
SQL
                ), [$newStatus, $newStatus, Translation::STATUS_SAVED, $group]);
            }
        } else {
            $this->translation->getConnection()->affectingStatement($this->adjustTranslationTable(<<<SQL
UPDATE $ltm_translations SET saved_value = "value", status = ?, is_auto_added = FALSE WHERE (COALESCE(saved_value,'') <> COALESCE("value",'') || (status <> ? and status <> ?))
SQL
...
WHERE NOT exists(SELECT * FROM $ltm_translations  tr WHERE tr."key" LIKE kt."key" AND tr."group" = kt."group" AND tr.locale = lt.locale)
      AND "key" LIKE ?
...
WHERE NOT EXISTS(SELECT * FROM $ltm_translations t WHERE t.locale = m.locale AND t."group" = m."group" AND t."key" LIKE m."key")
ORDER BY "key" ASC
SQL
    ...
    (SELECT DISTINCT mt."key", BINARY mt.ru ru, BINARY mt.en en
     FROM (SELECT lt."group", lt."key", group_concat(CASE lt.locale WHEN '$primaryLocale' THEN "VALUE" ELSE NULL END) en, group_concat(CASE lt.locale WHEN '$translatingLocale' THEN "VALUE" ELSE NULL END) ru
         ...
         JOIN (SELECT lt."group", lt."key", group_concat(CASE lt.locale WHEN '$primaryLocale' THEN "VALUE" ELSE NULL END) en, group_concat(CASE lt.locale WHEN '$translatingLocale' THEN "VALUE" ELSE NULL END) ru
               ...
               GROUP BY "group", "key") ht ON mt."key" LIKE ht."key"
     WHERE (mt.ru NOT LIKE ht.ru AND mt.en LIKE ht.en) OR (mt.ru LIKE ht.ru AND mt.en NOT LIKE ht.en)
    ) ft
        ON (lt.locale = '$translatingLocale' AND lt.value LIKE ft.ru) AND lt."key" LIKE ft.key
ORDER BY "key", "group"
SQL
        ));
    }
...
WHERE "group" = ? AND "key" LIKE ? AND locale IN (?) AND id NOT IN ($rowIds)
...
AND NOT exists(SELECT * FROM $ltm_translations t2 WHERE t2.value IS NOT NULL AND t2."group" = ? AND t1.locale = t2.locale
                AND t2."key" LIKE CONCAT(SUBSTR(t1."key", 1, CHAR_LENGTH(t1."key")-?), ?))
ORDER BY locale, "key"
